...
<div x-data="testimoniWidget()" x-init="init()" class="testimoni-widget-wrapper">
    ...
    <div x-show="dropdownOpen" x-cloak @click.away="dropdownOpen = false" class="testimoni-widget-dropdown">
        ...
        <div class="testimoni-dropdown-footer">
            @auth
                @php
                    $hasCompletedOrder = \App\Models\Order::where('user_id', Auth::id())->where('status', 'completed')->exists();
                @endphp
                ...
                @else
                    <button onclick="alert('Maaf, fitur testimoni hanya tersedia bagi pelanggan yang pesanan nya sudah selesai diproses.')" class="btn-give-testimoni" style="background-color: #9ca3af; cursor: not-allowed; opacity: 0.8;">
                        <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path>
                        </svg>
                        <span>Tulis Testimoni (Khusus Pelanggan)</span>
                    </button>
                @endif
            @else
